...
export function useMasonry<Item>({
  ...
  const { columnWidth, columnCount, range, estimateHeight, size, shortestColumn } = positioner;
  ...
  if (needsFreshBatch) {
    const batchSize = Math.min(
      ...
      Math.ceil(((scrollTop + overscanBy - shortestColumnSize) / itemHeightEstimate) * columnCount)
    );
...
const getContainerStyle = memoizeOne((isScrolling: boolean | undefined, estimateHeight: number) => ({
  position: 'relative',
  width: '100%',
  maxWidth: '100%',
...
const cmp2 = (args: IArguments, pargs: IArguments | any[]): boolean => args[0] === pargs[0] && args[1] === pargs[1];
