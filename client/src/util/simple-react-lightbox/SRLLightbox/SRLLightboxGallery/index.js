...
const SRLLightboxGallery = ({ options, callbacks, selectedElement, elements, dispatch }) => {
  // Context
  ...
  const { onCountSlides, onSlideChange, onLightboxClosed, onLightboxOpened } = callbacks
  ...
  const isIdle = useIdle(settings.hideControlsAfter < 1000 ? 9999999 : settings.hideControlsAfter)
  ...
  const establishNextOrPrevious = useCallback((selectedElementId, currentElementId, knownDirection) => {
  ...
  const handlePrevElement = useCallback(
    (elementID) => {
      ...
      const prevElement = elements[currentElementIndex - 1] || elements[elements.length - 1]

      // Set the state with the new element
      setElementAndDirection(elementID, null, prevElement, 'previous')

      // Callback
      const index = currentElementIndex - 1 === -1 ? elements.length - 1 : currentElementIndex - 1
      onChange({
        action: 'left',
        slides: {
  ...
  const handleNextElement = useCallback(
    (elementID) => {
      ...
      // Callback
      const index = currentElementIndex + 1 === elements.length ? 0 : currentElementIndex + 1
  ...
  useEffect(() => {
    ...
    if (!settings.disableKeyboardControls) {
      unsubscribe.current = subscribe(document, 'keydown', (e) => handleNavigationWithKeys(e.key), false)
    }
  ...
  return (
    <div ref={SRLStageRef} className="SRLStage">
      ...
      />
      <SRLContainerComponent {...selectedElement} {...controls} elements={elements} options={options} />
    </div>
  )
}
