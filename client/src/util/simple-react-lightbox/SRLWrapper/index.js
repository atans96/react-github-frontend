...
const SRLWrapper = ({ options, callbacks, images, children, defaultOptions, defaultCallbacks, customCaptions }) => {
  // Imports the context
  ...
  useEffect(() => {
    ...
    function dispatchGrabSettings(options, callbacks, customCaptions) {
      ...
      if (!isEqual(mergedSettings.options, context.options) || !isEqual(mergedSettings.callbacks, context.callbacks)) {
        context.dispatch({
          type: 'GRAB_SETTINGS',
          mergedSettings,
        });
    ...
    function handleImagesWithContext(array, elementType) {
      const elements = array.map((e, index) => {
        ...
        if (e.isLoaded) {
          ...
          const isGatsbyImage = e.img.offsetParent?.className.includes('gatsby-image-wrapper');
          const isGatsbyPicture = e.img.parentNode.localName !== 'picture';

          if (isGatsbyImage && (isBase64Image || isSVGImage) && isGatsbyPicture) {
            return;
          }
          ...
          const element = {
            ...
            source:
              elementType === 'IMG'
                ...
                : e.img.parentElement.href || e.img.offsetParent.parentElement.href || null,
    ...
    function handleDetectTypeOfElements(array) {
      ...
      const collectedDataAttributes = array.querySelectorAll("a[data-attribute='SRL']");
...
SRLWrapper.propTypes = {
  defaultOptions: PropTypes.shape({
    settings: PropTypes.shape({
      ...
      lightboxTransitionTimingFunction: PropTypes.oneOfType([PropTypes.string, PropTypes.array]),
      overlayColor: PropTypes.string,
      slideAnimationType: PropTypes.string,
      slideSpringValues: PropTypes.array,
      slideTransitionSpeed: PropTypes.number,
      slideTransitionTimingFunction: PropTypes.oneOfType([PropTypes.string, PropTypes.array]),
    }),
    ...
    caption: PropTypes.shape({
      ...
      captionFontWeight: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
