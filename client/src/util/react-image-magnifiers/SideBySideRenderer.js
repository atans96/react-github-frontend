...
const SideBySideRenderer = (props) => {
  const {
    itemPosition, // props receive from react-input-position
    active, // props receive from react-input-position
    elementDimensions, // props receive from react-input-position. the width and height are the wrapping
    // div of the original image
    elementOffset, // props receive from react-input-position
  ...
  if (fillAvailableSpace && !inPlace) {
    ...
    if (fillAlignTop) {
      ...
      const maxOffsetTop = availableHeight - elementOffset.top - (zoomContainerDimensions.height + fillGapBottom);
  ...
  const previewSize = {
    width: Math.floor(smallImageSize.width * (zoomContainerDimensions.width / (itemDimensions.width - 100))),
    height: Math.floor(smallImageSize.height * (zoomContainerDimensions.height / (itemDimensions.height - 100))),
  };
  ...
  position.x = utils.invertNumber(itemDimensions.width * -1 + zoomContainerSize.width, 0, position.x);
  position.y = utils.invertNumber(itemDimensions.height * -1 + zoomContainerSize.height, 0, position.y);
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div
        style={{
          ...styles.getZoomContainerStyle(zoomContainerSize.width, zoomContainerSize.height, inPlace, switchSides),
