...
export const MonthlyChart = ({ values, showPlusSymbol }: any) => {
  if (values.length === 0) return <div style={{ fontStyle: 'italic', marginBottom: '.5rem' }}>No data available</div>;

  const last12Months = getLas12Months();
  const findDeltaByMonth = ({ year, month }: any) => {
    const foundValue = values.find((value: any) => value.year === year && value.month === month);
  ...
  const monthlyDeltaMax = Math.max(...monthlyDeltas.filter((value) => value !== undefined));

  return (
    <div>
      <div className="project-chart">
        <div className="project-chart-columns">
          {monthlyDeltas.map((value, idx) => (
            <div key={idx} className="project-chart-column">
              <div
        ...
        <div className="project-chart-months">
          {months.map((monthName, idx) => (
            <div key={idx} className="project-chart-month">
              {monthName}
            </div>
          ))}
        </div>
...
function formatDelta(value: number, { decimals = 0, showPlusSymbol = false }) {
  const numberFormat = decimals === 0 || value < 1000 ? '0' : `0.${'0'.repeat(decimals)}`;
