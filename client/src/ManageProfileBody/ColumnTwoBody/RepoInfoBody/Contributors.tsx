...
const Contributors: React.FC<Props> = ({ fullName, openContributors }) => {
  const { contributors } = ManageProfileStore.store().Contributors();
  const [contributionRepo, setContributionRepo] = useState<ContributorProps[]>([]);
  const location = useLocation();
  useDeepCompareEffect(() => {
    let isFinished = false;
    if (contributors.length > 0 && location.pathname === '/profile' && !isFinished) {
      const contribution = contributors.find((xx: ContributorsProps) => fullName === xx.fullName);
      if (contribution) {
