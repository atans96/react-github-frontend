...
const Results = Loadable({
  loading: Empty,
  delay: 300,
  loader: () => import(/* webpackChunkName: "Results" */ './PureSearchBarBody/Results'),
});

const PureInput = Loadable({
  loading: Empty,
  delay: 300,
  loader: () => import(/* webpackChunkName: "PureInput" */ './PureInput'),
});
          ...
          dispatch({
            ...
            payload: {
              filteredMergedData: fastFilter((x: MergedDataProps) => {
                ...
                return (
                  [...topics, ...state.filteredTopics].reduce((a: any, b: any) =>
                    a.filter((c: string) => b.includes(c))
                  ).length > 0
                );
