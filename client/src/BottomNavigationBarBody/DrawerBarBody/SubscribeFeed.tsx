            ...
            if (result.items) {
              ...
              try {
                ...
                if (!openSubscription) {
                  ...
                  watchUsersFeedsAdded({
                    ...
                    .then((res: any) => {
                      if (res.data.watchUsersFeedsAdded && res.data.watchUsersFeedsAdded.login.length > 0) {
                        const temp = res.data.watchUsersFeedsAdded.login.find((x: any) => x.login === obj.login).feeds;
                  ...
                  const feeds = uniqFast(fastFilter((x: any) => x.login === obj.login, HTML).map((x: any) => x.feeds));
                  watchUsersFeedsAdded({
                    variables: {
                      login: obj.login,
                      feeds: feeds,
              ...
              <Then>
                <React.Fragment>
                  <Collapse in={showMore} timeout={0.1} unmountOnExit>
                    {xmlFileListAppend.slice(5).map((xml: string, idx: number) => {
                      if (displayUnseenFeeds.includes(xml)) {
                        return (
                          <Paper className={classes.paperUnseen} key={idx}>
                            <div dangerouslySetInnerHTML={{ __html: xml }} />
