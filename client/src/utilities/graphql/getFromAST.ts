import { DocumentNode, OperationDefinitionNode, FragmentDefinitionNode, ValueNode } from 'graphql';
...
export function getFragmentDefinitions(doc: DocumentNode): FragmentDefinitionNode[] {
  return doc.definitions.filter((definition) => definition.kind === 'FragmentDefinition') as FragmentDefinitionNode[];
}
...
export function getDefaultValues(definition: OperationDefinitionNode | undefined): Record<string, any> {
  ...
  if (defs && defs.length) {
    defs.forEach((def) => {
      if (def.defaultValue) {
        valueToObjectRepresentation(defaultValues, def.variable.name, def.defaultValue as ValueNode);
      }
    });
  }
