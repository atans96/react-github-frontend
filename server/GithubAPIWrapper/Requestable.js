    ...
    return this._request("GET", path, options)
      .then((response) => {
        ...
        if (response.headers.link) {
          if ("last" in getPaginationInfo(response.headers.link)) {
            response.paginationInfo = getPaginationInfo(response.headers.link);
          } else {
            response.paginationInfo = {
              last: last - 1,
            };
          }
          if (parseInt(options.per_page) > 100) {
            this.multiplier += 1;
            const stillHasNextPage =
              parseInt(options.per_page) - this.multiplier * 100 > 0;
...
function getPaginationInfo(linksHeader = "") {
  ...
  return links.reduce(function (parsed_data, link) {
    parsed_data[link.match(/rel="(.*?)"/)[1]] = link
      .match(/<(.*)>/)[1]
      .match(/page=(.*?)\&/)[1];
    return parsed_data;
  }, {});
