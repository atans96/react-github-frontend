...
class User extends Requestable {
  ...
  __getScopedUrl(endpoint) {
    if (this.__user) {
      return endpoint // /users/this.__user/starred
  ...
  /**
   * List the watched Repos from the user
   * @param {Requestable.callback} [cb] - will receive the list of organizations
   * @return {Promise} - the promise for the http request
   */
  listWatchedRepos(options, cb) {
    let requestOptions = this._getOptionsWithDefaults(options);
    return this._requestAllPages(
      this.__getScopedUrl("subscriptions"),
      requestOptions,
      cb
    );
  }
  ...
  listNotifications(options, cb) {
    ...
    return this._request(
      "GET",
      this.__getScopedUrl("notifications"),
      options,
      cb
    );
  }
   ...
   * @return {Promise} - the promise for the http request
   */
  listStarredRepos(options, cb) {
    let requestOptions = this._getOptionsWithDefaults(options);
    return this._requestAllPages(
      this.__getScopedUrl("starred"),
      requestOptions,
      cb
    );
  }
